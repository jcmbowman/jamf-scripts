...
# the computer extension attribute scripts stored on a Jamf Pro server then do the following:
#
# 1. Download the script as JSON
# 2. Identify the script name
# 3. Extract the script contents from the downloaded JSON
# 4. Save the script to a specified directory

# If setting up a specific API account with limited rights, here are the required API privileges
# for the account on the Jamf Pro server:
#
# computer info lookup, Extension Attribute R/O, update computer
...
case $hostname in
     HOSTNAME1.local)
          ScriptDownloadDirectory="/Users/<username>/CodeProjects/jamf-scripts/jamf-extension-attribute-scripts"
          ;;
    HOSTNAME2.local)
          ScriptDownloadDirectory="/Users/<username>/Code Projects/jamf-scripts/jamf-extension-attribute-scripts"
          ;;
     *)
...
checkTokenExpiration() {
    ...
    if [[ $token_expiration_epoch -ge $current_epoch ]]; then
		a=0 #echo "Token valid until the following epoch time: " "$token_expiration_epoch"
    else
...
echo "Getting list of extension attributes from ${jamfpro_url}"
checkTokenExpiration

Script_id_list=$(curl -ks -X 'GET' \
     "${jamfpro_url}/api/v1/computer-extension-attributes?page=0&page-size=10000&sort=name.asc" \
     -H "Authorization: Bearer $access_token" 2>/dev/null)
Script_id_count=$( json_value "$Script_id_list" 'results' '=')

# echo "$Script_id_list"
# echo "$Script_id_count"

for ((count=0; count<"$Script_id_count"; count++)); do
     EAtype=$( json_value "$Script_id_list" 'results' $count 'inputType')
     # Echo "ListCount: $count, EAtype Found: $EAtype"
     if [ "$EAtype" == "SCRIPT" ]; then
          # Echo "Found Script!"
          EAname=$( json_value "$Script_id_list" 'results' $count 'name' | sed -e 's|:|(colon)|g' -e 's/\//_/g' )
          # echo "EAname: ${EAname}"

          EAscript=$( json_value "$Script_id_list" 'results' $count 'scriptContents')
          #echo "EAScript: ${EAscript}"

          EAscriptZsh=$( echo "$EAscript" | head -n 1 | awk "{print $1}" | grep "/zsh")     
          EAscriptBash=$( echo "$EAscript" | head -n 1 | awk "{print $1}" | grep -i -e "/sh" -e "/bash")     
          EAscriptPython=$( echo "$EAscript" | head -n 1 | awk "{print $1}" | grep -i -e "/python")     
          if [ "$EAscriptZsh" != "" ]; then
               EAscriptSuffix="zsh"
               # echo "ZSH? ${EAscriptZsh}"
          elif [ "$EAscriptBash" != "" ]; then
               EAscriptSuffix="sh"
               # echo "BASH? ${EAscriptBash}"
          elif [ "$EAscriptPython" != "" ]; then
               EAscriptSuffix="py"
               # echo "PYTHON? ${EAscriptPython}"
          fi

          EAname="${EAname}.${EAscriptSuffix}"

	
     	## Save the downloaded script 
          echo "${EAname}       - Script being downloaded."
	     echo "Saving ${EAname} file to $ScriptDownloadDirectory."


	     echo "$EAscript" | sed -e 's/&lt;/</g' -e 's/&gt;/>/g' -e 's/&quot;/"/g' -e 's/&amp;/\&/g' > "${ScriptDownloadDirectory}/${EAname}"


     #      echo "Loop spin:" $count
     fi
done
